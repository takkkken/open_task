...
global $GLADIUS_DB_ROOT,$GLADIUS_BOOL_OPS, $GLADIUS_ERRORS, $GLADIUS_OPS;
...
class Gladius extends Gladius_Core {
	...
	function Gladius($db = null) {
		...
		$this->db_root = $GLOBALS['GLADIUS_DB_ROOT'];
		if (isset($db))
			$this->SelectDB($db);
	}
	...
	/* public */ function SelectDB($dbname) {
		$this->_command = 'USE';
		$old_db = $this->db_root.$this->database;
		$ch = $dbname{strlen($dbname)-1};
		if ($ch=='/' || $ch =='\\')
			$dbname = substr($dbname,0,-1);

		$p=strrpos($dbname, '/');
		if ($p===false)	$p = strrpos($dbname, '\\');
		if ($p===false) {
			$this->database = $dbname.'/';
//			$this->db_root = $GLOBALS['GLADIUS_DB_ROOT'];
		} else {
			$this->database = substr($dbname, $p+1);
			$this->db_root = dirname($dbname).'/'; // danger here for PHP < 4.3.0
		}
		if (!is_dir($this->db_root.$this->database)) {
			$this->_error(_G__CANNOT_SELECT_DB, $dbname);
			return false;
		}

		if ($old_db != $this->db_root.$this->database) {
			$this->_reset();
			$this->_command = 'USE';
		}
