...
/* currently unused - parked here
class Gladius_Resultset_empty
{
	var $fields = false;
	var $EOF = true;
	function MoveNext($rowNumber) {return false;}
	function Move($rowNumber) {return false;}
	function MoveFirst($rowNumber) {return false;}
	function MoveLast($rowNumber) {return false;}
...
/*				echo '<pre>';var_dump($current->next);echo '</pre>';die;
			...
			if (preg_match('/\\s*(order|group|limit|'.$last_exp.')/iA', $this->sql, $ignored, 0, $this->offset)) {
				break;
			}
...
//		echo ' '.$raw_operator.' ('.get_class($right).')'.$right->content.')';
	...
	function _parse_limit() {
		if (preg_match('/\\s*limit\\s+(\\d+)\\s*,?\\s*([\\+\\-]?\\d+)?/iA',
			$this->sql, $m, PREG_OFFSET_CAPTURE, $this->offset)) {
...
//							die('GRTE: asterisk in aggregate functions not yet supported');
	...
	function _parse_function($func, &$obj, $fi) {
		...
		$obj->func = gladius_strtolower($m[1]);
		
		if (!function_exists('gladius_sql_'.$obj->func)) {
			die("GRTE: invalid aggregate function ".$obj->func);
		}
	...
	function _load_fields(&$fields,&$aliases) {
		...
		foreach ($fields as $fld) {
			...
			if ($fld[2]) {	// this is a function call
